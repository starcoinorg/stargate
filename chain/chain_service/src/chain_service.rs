...
impl ChainService {
    ...
    pub fn get_transaction_by_seq_num_inner(&self, account_address: AccountAddress, seq_num: u64) -> Result<Option<SignedTransactionWithProof>> {
        ...
        let list = read_db.get_transactions(version, 1, version, false)?;
        let mut infos = list.transaction_and_infos;
        let (txn, info) = infos.pop().expect("transaction_and_infos is none.");
        let first_proof = list.proof_of_first_transaction.expect("proof_of_first_transaction is none.");
        let proof = SignedTransactionProof::new(first_proof, info);
        let txn_proof = SignedTransactionWithProof { version, signed_transaction: txn, events: None, proof };
        Ok(Some(txn_proof))
    }
}
