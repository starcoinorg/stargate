...
use libra_config::config::{ConsensusPeerInfo, SeedPeersConfig, NetworkPeerInfo, ConsensusType, NetworkConfig, NodeConfig, RoleType, TestConfig};
...
use tokio::time::{Interval, Instant, interval_at};
...
//            conf_1.networks[0].peer_id.clone(),
//            NetworkPeerInfo {
//                signing_public_key: network_signing_public_key_1,
//                identity_public_key: network_identity_pubkey_key_1,
//            },
//        ),
//        (
//            conf_2.networks[0].peer_id.clone(),
//            NetworkPeerInfo {
//                signing_public_key: network_signing_public_key_2,
//                identity_public_key: network_identity_pubkey_key_2,
//            },
//        ),
//    ]
//    .into_iter()
...
fn test_pow_single_node() {
    ::libra_logger::init_for_e2e_testing();
    //let mut conf_1 = node_random_conf(true, "/memory/0", 0);
    let mut conf_1 = gen_node_config_with_genesis(1, true);
    print_ports(&conf_1);
    debug!("conf1:{:?}", conf_1);
    let _handle_1 = setup_environment(&mut conf_1, false);
...
fn commit_tx(port: u32, executor: Handle) -> Sender<()> {
    ...
    let account = gen_account();
    let faucet_executor = executor.clone();
    let task = interval_at(Instant::now(), Duration::from_secs(10))
        .take_while(move |_| match shutdown_receiver.try_recv() {
            Err(_) | Ok(Some(_)) => {
                info!("Build block task exit.");
                future::ready(false)
...
fn test_validator_conf() {
    let conf_1 = gen_node_config_with_genesis(1, true);
    let conf_2 = gen_node_config_with_genesis(2, true);
...
#[test]
fn test_genesis() {
    let conf = gen_node_config_with_genesis(1, false);
    println!("{:?}", conf);
}

fn gen_node_config_with_genesis(times: usize, network_random: bool) -> NodeConfig {
