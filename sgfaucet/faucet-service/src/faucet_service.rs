use faucet_proto::proto::faucet::{create_sg_faucet, FaucetRequest as FaucetRequestProto, EmptyResponse, SgFaucet};
use grpc_helpers::{provide_grpc_response, spawn_service_thread_with_drop_closure, ServerHandle};
use anyhow::Result;
use std::{convert::{TryFrom, TryInto}, pin::Pin, sync::Arc, thread, clone::Clone, fs};
use sgchain::star_chain_client::{StarChainClient, ChainClient};
...
use faucet_config::{FaucetConf, load_faucet_conf};
use base64::decode;
...
impl FaucetNode {
    pub fn load_and_run() -> ServerHandle {
        let faucet_conf = load_faucet_conf();
...
impl SgFaucetService {
    pub fn new(faucet_conf: FaucetConf) -> Self {
        ...
        let key_base64 = decode(&fs::read(pri_file).expect("read pri file err.")).expect("base64 decode err.");
        let pri_key = Ed25519PrivateKey::try_from(key_base64.as_ref()).expect("Ed25519PrivateKey parse err.");
//        let key_pair = generate_keypair::load_key_from_file(
//            pri_file,
//        ).expect("Faucet account key is required to generate config");

        let key_pair = ConsensusKeyPair::load(pri_key).take_private().expect("pri key is none.");

        SgFaucetService{chain_client, key_pair: Arc::new(key_pair)}
    }
}

impl SgFaucet for SgFaucetService {
    fn faucet(&mut self, ctx: ::grpcio::RpcContext, req: FaucetRequestProto, sink: ::grpcio::UnarySink<EmptyResponse>) {
...
//        let f = async move {
//            chain_client.faucet_with_sender(None, self.key_pair.as_ref(), faucet_req.address, faucet_req.amount).await
//        };
