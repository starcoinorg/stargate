...
use network_libp2p::{CustomMessage, Multiaddr, multiaddr::Protocol, ServiceEvent, build_multiaddr};
	...
	where TMsg: CustomMessage + Send + 'static
{
	...
	for index in 0 .. num {
		...
		let proto = network_libp2p::RegisteredProtocol::new(&b"tst"[..], &[1]);
		result.push(network_libp2p::start_service(config, proto).unwrap().0);
	}
