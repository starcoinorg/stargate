...
use slots::{SlotState, SlotType, Slots};
...
impl Peerset {
    ...
    pub fn from_config(config: PeersetConfig) -> (Peerset, PeersetHandle) {
        ...
        for peer_id in config.reserved_nodes {
            peerset
                .data
                .discovered
                .add_peer(peer_id, SlotType::Reserved);
        }
    ...
    fn on_add_reserved_peer(&mut self, peer_id: PeerId) {
        ...
        match self.data.out_slots.add_peer(peer_id, SlotType::Reserved) {
            ...
            SlotState::Swaped { removed, added } => {
                ...
                self.data
                    .discovered
                    .add_peer(removed.clone(), SlotType::Common);
                // swap connections
    ...
    fn on_remove_reserved_peer(&mut self, peer_id: PeerId) {
        ...
        if self.data.reserved_only {
            if self.data.in_slots.clear_slot(&peer_id) || self.data.out_slots.clear_slot(&peer_id) {
                ...
                self.data
                    .discovered
                    .add_peer(peer_id.clone(), SlotType::Common);
    ...
    fn on_set_reserved_only(&mut self, reserved_only: bool) {
        ...
        if self.data.reserved_only {
            for peer_id in self
                ...
                .clear_common_slots()
                .into_iter()
                .chain(self.data.out_slots.clear_common_slots().into_iter())
            {
                // insert peer back into discovered list
                self.data
                    .discovered
    ...
    fn alloc_slots(&mut self) {
        while let Some((peer_id, slot_type)) =
            self.data.discovered.pop_peer(self.data.reserved_only)
        {
    ...
    pub fn dropped(&mut self, peer_id: PeerId) {
        ...
        if self.data.in_slots.is_connected_and_reserved(&peer_id)
...
#[cfg(test)]
mod tests {
    use super::{IncomingIndex, Message, Peerset, PeersetConfig};
    use futures::prelude::*;
    use libp2p::PeerId;
    ...
    fn next_message(peerset: Peerset) -> Result<(Message, Peerset), ()> {
        let (next, peerset) = peerset.into_future().wait().map_err(|_| ())?;
    ...
    fn test_peerset_from_config_with_bootnodes() {
        ...
        assert_messages(
            peerset,
            vec![Message::Connect(bootnode), Message::Connect(bootnode2)],
        );
    }
    ...
    fn test_peerset_remove_reserved_peer() {
        ...
        assert_messages(
            peerset,
            vec![Message::Drop(reserved_peer), Message::Drop(reserved_peer2)],
        );
    }
    ...
    fn test_peerset_report_peer() {
        ...
        assert_messages(
            peerset,
            vec![Message::Connect(bootnode.clone()), Message::Drop(bootnode)],
        );
    }
    ...
    fn test_peerset_dropped() {
        ...
        let mut peerset = assert_messages(
            peerset,
            vec![
                Message::Connect(reserved_peer.clone()),
                Message::Connect(bootnode.clone()),
        ...
        let _peerset = assert_messages(
            peerset,
            vec![Message::Connect(reserved_peer), Message::Connect(bootnode2)],
        );
    }
