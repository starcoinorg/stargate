...
use sparse_merkle::{node_type::Node, RetiredRecordType, RetiredStateRecord, TreeUpdateBatch};
...
impl TreeReader for MockTreeStore {
    ...
    fn get_blob(&self, blob_hash: HashValue) -> Result<Blob> {
        Ok(self
            .0
            .read()
            .unwrap()
...
impl MockTreeStore {
    ...
    fn put_blob(&self, key: HashValue, blob: Blob) -> Result<()> {
    ...
    pub fn write_tree_update_batch(&self, batch: TreeUpdateBatch) -> Result<()> {
        ...
        blob_batch
            .into_iter()
            .map(|(k, v)| self.put_blob(k, Blob::from(v)))
            .collect::<Result<Vec<_>>>()?;
